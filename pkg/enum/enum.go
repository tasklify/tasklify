...
// <-- Copied from github.com/orsinium-labs/enum -->  //
...
func (e Enum[M, V]) Index(member M) int {
	for i, m := range e.members {
		if e.WrappedValue(m) == e.WrappedValue(member) {
			return i
		}
	}
...
func (e Enum[M, V]) Values() []V {
	...
	for _, m := range e.members {
		res = append(res, e.WrappedValue(m))
	}
	return res
}
...
func (e Enum[M, V]) String() string {
	...
	for _, m := range e.members {
		values = append(values, fmt.Sprintf("%v", e.WrappedValue(m)))
	}
...
func (e Enum[M, V]) GoString() string {
	values := make([]string, 0, len(e.members))
	for _, m := range e.members {
		values = append(values, fmt.Sprintf("%T{%#v}", m, e.WrappedValue(m)))
	}
